          ...
          <AnimatePresence>
            {mobileMenuOpen && (
              <motion.div
                ...
                className="fixed top-14 left-4 right-4 z-40 bg-background/95 backdrop-blur-md border border-border rounded-xl shadow-lg p-3"
              >
                <div className="flex flex-col gap-1">
                  {navItems.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => handleNavClick(item.id)}
                      className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium text-left transition-all duration-300 ${
                        activeSection === item.id
                          ? 'bg-primary text-primary-foreground'
